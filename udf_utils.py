...
def extract_requirement(file_content):
    try:
        requirement_match = re.search(r'(REQUIREMENTS?/\s?MINIMUM QUALIFICATIONS?)(.*)(PROCESS_NOTES?)', file_content, re.DOTALL)
        req = requirement_match.group(2).strip() if requirement_match else None
        return req
    except Exception as e:
        raise ValueError(f'Error extracting requirement: {str(e)}')
...
def extract_experience_length(file_content):
    try:
        experience_length_match = re.search(
            r'(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten|one|two|three|four|five)\s(years?)\s(of\sfull(-|\s)time)',file_content)
...
def extract_education_length(file_content):
    try:
        education_length_match = re.search(
            r'(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten|one|two|three|four|five)(\s|-)(years?)\s(college|university)',file_content)
